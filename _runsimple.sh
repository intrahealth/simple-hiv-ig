...
# https://github.com/DBCG/cqf-ruler/issues/533
...
for FILE in FHIRHelpers FHIRCommon EasyLib ; do curl -X PUT -H "$HEADER" --data @output/Library-${FILE}.json $FHIR/Library/${FILE} | jq .issue ; done
...
curl -H "$HEADER" $FHIR'/Measure/Easy/$evaluate-measure?periodStart=1960-01-01&periodEnd=2022-12-31&reportType=population' | jq . 
